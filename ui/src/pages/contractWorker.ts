import { Mina, PublicKey, fetchAccount, Field, UInt64, AccountUpdate, PrivateKey, UInt8, Bool } from 'o1js';
...
const functions = {
  setActiveInstance: async (networkObj: { network: string } | string = 'Devnet') => {
    let network: string;
  ...
  loadContract: async (args: {}) => {
    const { FungibleToken } = await import('../../../contracts/build/src/FungibleToken.js');
    const { FungibleTokenAdmin } = await import('../../../contracts/build/src/FungibleTokenAdmin.js');
  ...
  fetchTokenAccount: async (args: { publicKey58: string}) => {
    ...
    let response:any = await fetchAccount({ publicKey:publicKey, tokenId:tokenId });
    return response;
  },
  ...
  createTransferTransaction: async (args: { from58: string, to58: string, amount: string }) => {
  ...
  mintToken: async (args: { sender: string, recipientAddress58: string, amount: string, adminPrivateKey58: string, feePayerPrivateKey58: string }) => {
    try {
      ...
      const feePayerPrivateKey = PrivateKey.fromBase58(args.feePayerPrivateKey58); // Convert fee payer private key
  ...
  deployToken: async (args: { 
    ...
    try {
      console.log("Deploying token", args.tokenSymbol, args.tokenDecimals, args.src);

      const network = args.network || 'Devnet';
      const nonce = await getInferredNonce(feePayer.publicKey.toBase58(), network);
      ...
      const deployTx = await Mina.transaction({
        ...
        await token.deploy({
          ...
          src: args.src || "https://github.com/MinaFoundation/mina-fungible-token/blob/main/FungibleToken.ts",
        });
